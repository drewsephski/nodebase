 ...
 *
 * @param provider - The LLM provider ('anthropic', 'openai', 'groq', 'openrouter')
...
export async function getConfiguredProviders(userId?: string): Promise<string[]> {
  const providers: ('anthropic' | 'openai' | 'groq' | 'openrouter')[] = ['anthropic', 'openai', 'groq', 'openrouter'];
...
export async function getProvidersStatus(userId?: string): Promise<{
  ...
  openrouter: { configured: boolean; source: 'user' | 'env' | null };
}> {
  const status: any = {};

  for (const provider of ['anthropic', 'openai', 'groq', 'openrouter'] as const) {
